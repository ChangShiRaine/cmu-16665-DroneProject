function [] = plot_quadrotor_cumulative_errors(state, state_des,time_vector)
time_step = 0.005;
...
% Plot position error
labels = {'x [m]', 'y [m]', 'z [m]'};
title_name = {'Accumulative Error in x','Accumulative Error in y','Accumulative Error in z'};
str = 'Plot of Accumulative Error';
figure('Name',str);

for i = 1:3
    subplot(5, 3, i)
    plot(time_vector,time_step * cumsum(abs(error_pos(i,:))),'r');
    grid on
    xlabel('time [s]')
    ylabel(labels{i})
    title(title_name{i})
...
labels = {'\phi', '\theta', '\psi'};
title_name = {'Accumulative Error in \phi','Accumulative Error in \theta','Accumulative Error in \psi'};

for i = 1:3
    subplot(5, 3, i+3)
    plot(time_vector,time_step * cumsum(abs(error_rpy(i,:))),'r');
    grid on
    xlabel('time [s]')
    ylabel(labels{i})
    title(title_name{i})
...
title_name = {'Accumulative Error in v_x','Accumulative Error in v_y','Accumulative Error in v_z'};

for i = 1:3
    subplot(5, 3, i+6)
    plot(time_vector,time_step * cumsum(abs(error_vel(i,:))),'r');
    grid on
    xlabel('time [s]')
    ylabel(labels{i})
    title(title_name{i})
...
title_name = {'Accumulative Error in \omega_x','Accumulative Error in \omega_y','Accumulative Error in \omega_z'};

for i = 1:3
    subplot(5, 3, i+9)
    plot(time_vector,time_step * cumsum(abs(error_ang_vel(i,:))),'r');
...
title_name = {'Accumulative Error in a_x','Accumulative Error in a_y','Accumulative Error in a_z'};

for i = 1:3
    subplot(5, 3, i+12)
    plot(time_vector,time_step * cumsum(abs(error_acc(i,:))),'r');
    grid on
    xlabel('time [s]')
    ylabel(labels{i})
    title(title_name{i})
...
%     %stepinfo(pos(i,:),time_vector,pos_des(i,:))
%     grid on
%     xlabel('time [s]')
%     ylabel(labels{i})
%     title(title_name{i})
