...
% Set initial desired state for question 5 & 6.5 (z_initial = 1)
if question == 5 || question == 6.5
    actual_desired_state_matrix(3,1) =  trajectory_matrix(3,1);
end
...
%% get the rise time, settling time, percent overshoot
if question == 2 || question == 3 || question == 5 || question == 6.2 || question == 6.3 || question == 6.5
    z_position = actual_state_matrix(3,:);
    z_velocity = actual_state_matrix(6,:);
    yaw = actual_state_matrix(9,:);
    t = time_vec;
    z_position_perform = stepinfo(z_position,t,'SettlingTimeThreshold', 0.01,'RiseTimeThreshold',[0 0.9])
    z_velocity_perform = stepinfo(z_velocity,t,'SettlingTimeThreshold',0.01,'RiseTimeThreshold',[0 0.9])
    %yaw_perform = stepinfo(yaw,t,'SettlingTimeThreshold',0.01, 'RiseTimeThreshold',[0 0.9])
end

%%
if question == 8 || question == 9
    plot_quadrotor_cumulative_errors(actual_state_matrix, actual_desired_state_matrix, time_vec)
end
